...
export default function CLearningMaterial() {
  ...
  const learningContent = {
    "hello-world": {
      ...
      intro: "This module introduces you to the very basics of C programming. You will learn how to set up a C environment and write your first C program that outputs 'Hello, World!' to the console.",
...
int main() {
  ...
  if (quizResult) {
    ...
    if (!completedQuizzes.includes(lessonId)) {
      ...
      const nextContentIndex = contentList.findIndex(content => content.id === lessonId) + 1;
  ...
  function downloadPDF() {
    ...
    doc.setFontSize(12);
    doc.text(lesson.intro, 20, 30);
    doc.setFontSize(12);
    doc.text("Code Example:", 20, 50);
    doc.setFont("courier"); // Set font to Courier for code example
    doc.setFontSize(11);
    doc.text(lesson.example, 20, 60);
  ...
  return (
    <div className="min-h-screen flex bg-gradient-to-br from-black via-gray-900 to-yellow-700 text-yellow-50">
      <aside className="w-1/4 p-4 bg-gray-900 border-r border-yellow-600">
        <h2 className="text-2xl font-bold mb-4 text-yellow-300">C Language Topics</h2>
        <ul className="divide-y divide-yellow-600">
          {contentList.map((content) => (
            <li key={content.id} className={`flex items-center justify-between p-2 rounded hover:bg-yellow-600 transition-colors ${content.id === lessonId ? "bg-yellow-600 text-black" : "text-yellow-200"}`}>
              {content.unlocked ? (
              ...
              {completedQuizzes.includes(content.id) && <FaCheck className="text-green-500" />}
            </li>
          ))}
        </ul>
      </aside>
      <main className="flex-1 p-8">
        {lesson ? (
          <div className="max-w-3xl mx-auto bg-black border border-yellow-500 rounded-lg shadow-lg p-6">
            <h1 className="text-4xl font-bold mb-4 text-yellow-400">{lesson.title}</h1>
            <section>
              <h2 className="text-2xl font-semibold mb-2 text-yellow-300">Introduction</h2>
              <p className="text-yellow-200 mb-4">{lesson.intro}</p>
              <h2 className="text-2xl font-semibold mb-2 text-yellow-300">Code Example</h2>
