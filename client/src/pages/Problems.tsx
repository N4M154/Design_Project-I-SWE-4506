...
function Problems() {
  ...
  const handleSubmit = (problemId: number) => {
    ...
    const randomResult = results[Math.floor(Math.random() * results.length)] as 'AC' | 'WA' | 'TLE';
  ...
  const getStats = () => {
    const solved = new Set(submissions.filter(s => s.status === 'AC').map(s => s.problemId)).size;
          ...
          <div className="mb-8 p-6 bg-yellow-50 rounded-lg shadow-sm">
            ...
            <div className="grid grid-cols-2 gap-4">
              ...
              <div className="p-4 bg-white rounded-lg">
                ...
                <p className="text-3xl font-bold text-blue-600">{stats.attempted}</p>
              </div>
            </div>
          </div>
                  ...
                  >
                    <div className="flex items-center justify-between">
                      ...
                      <div className="flex items-center gap-2">
                        {getStatusIcon(submissions.find(s => s.problemId === problem.id)?.status || null)}
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <div className="mt-8 bg-white rounded-lg shadow-sm p-6">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <pre className="mt-2 bg-gray-100 p-3 rounded">{selectedProblem.sampleInput}</pre>
                </div>
                <div>
                  <h3 className="font-semibold">Sample Output</h3>
                  <pre className="mt-2 bg-gray-100 p-3 rounded">{selectedProblem.sampleOutput}</pre>
                </div>
                <div>
